...
var createIncompleteList = function createIncompleteList(text) {
  ...
  completeButton.addEventListener("click", function () {
    ...
    deleteFromIncompleteList(completeButton.closest("li")); // 完成したTODOに追加する要素を取得する

    var addTarget = completeButton.closest("li"); // TODO内容テキストを取得

    var text = addTarget.getElementsByClassName("todo-item")[0].innerText; // divタグ以下は削除

    addTarget.childNodes[0].textContent = null; // pタグの生成

    var p = document.createElement("p");
    p.className = "todo-item";
    p.innerText = text; // buttonタグを生成

    var backButton = document.createElement("button");
    backButton.innerText = "戻る";
    backButton.addEventListener("click", function () {
      // 押された完了ボタンのliを未完了リストから削除する
      deleteFromCompleteList(backButton.closest("li")); // テキスト取得
    ...
    var completeArea = document.getElementById("complete-list");
  ...
  li.appendChild(div); // divタグの子要素にpタグを設定する

  div.appendChild(p); // divタグの子要素にbuttonタグを設定する
